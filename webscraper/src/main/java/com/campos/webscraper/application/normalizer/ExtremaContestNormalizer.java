...
/**
 * Maps Extrema municipal contest previews into the canonical public contest entity shape.
 */
@Component
public class ExtremaContestNormalizer {

    private static final Pattern DURABLE_IDENTIFIER_PATTERN =
            Pattern.compile("(?i)(?:edital|processo seletivo|concurso|contrata[cç][aã]o).{0,40}?(\\d+/\\d{4})");

    private final ObjectMapper objectMapper;

    public ExtremaContestNormalizer() {
        this(new ObjectMapper().findAndRegisterModules());
    }

    public ExtremaContestNormalizer(ObjectMapper objectMapper) {
        this.objectMapper = Objects.requireNonNull(objectMapper, "objectMapper must not be null");
    }

    public PublicContestPostingEntity normalize(ExtremaContestPreviewItem item, LocalDateTime fetchedAt) {
        Objects.requireNonNull(item, "item must not be null");
        Objects.requireNonNull(fetchedAt, "fetchedAt must not be null");

        if (item.editalUrl() == null || item.editalUrl().isBlank()) {
            throw new IllegalArgumentException("Extrema contest item must have a stable edital URL");
        }
    ...
    private String toJson(ExtremaContestPreviewItem item) {
        try {
            return objectMapper.writeValueAsString(toPayload(item));
        } catch (JsonProcessingException exception) {
            throw new IllegalStateException("Failed to serialize Extrema preview payload for audit", exception);
        }
    }

    private Map<String, Object> toPayload(ExtremaContestPreviewItem item) {
    ...
    private List<Map<String, String>> toAttachmentPayload(List<ExtremaContestAttachment> attachments) {
        if (attachments == null) {
            return List.of();
        }
    ...
    private String buildStableContestId(ExtremaContestPreviewItem item) {
        ...
        String normalized = Normalizer.normalize(seed, Normalizer.Form.NFD)
                ...
                .replaceAll("[^a-z0-9]+", "-")
                .replaceAll("^-+|-+$", "");
        return "municipal_extrema:" + normalized;
    }

    private String resolveDurableIdentifier(ExtremaContestPreviewItem item) {
    ...
    private LocalDate resolvePublishedAt(ExtremaContestPreviewItem item, LocalDateTime fetchedAt) {
    ...
    private ContestStatus resolveContestStatus(ExtremaContestPreviewItem item, LocalDateTime fetchedAt) {
        LocalDate today = fetchedAt.toLocalDate();
        if (item.registrationEndDate() != null) {
            if (!item.registrationEndDate().isBefore(today)) {
                return ContestStatus.OPEN;
            }
            if (item.examDate() != null && !item.examDate().isBefore(today)) {
                return ContestStatus.EXAM_SCHEDULED;
            }
            return ContestStatus.REGISTRATION_CLOSED;
        }
        if (item.examDate() != null) {
            if (!item.examDate().isBefore(today)) {
                return ContestStatus.EXAM_SCHEDULED;
            }
            return ContestStatus.REGISTRATION_CLOSED;
        }
